...
import { VOTE,GET_PROP_INFO,CHANGE_VOTE_ID,BREAD_SHOW,CHANGE_CHECKED_OPTIONS,BATCH_VOTE_PROP,REST_DATA } from './vuex.js';
...
export default {
    ...
    methods: {
        getTableData() {
            ...
            this.$store.dispatch(`${VOTE}/${GET_PROP_INFO}`,sendData).then(() => {
                // this.getTableHeight();
            });
        },
        getTableHeight() {
        ...
        sureVote() {
            this.$refs.batchVoteInfo.validate(valid => {
                if (valid) { 
                    ...
                    this.$store.dispatch(`${VOTE}/${BATCH_VOTE_PROP}`,arr).then(() => {
                        this.handleClose();
                        this.getTableData();
                    });
                }
